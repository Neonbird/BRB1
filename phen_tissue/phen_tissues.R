...
# df[,1] <- gsub(".fgseaRes.csv", "", my.files)
df[,1] <- my.files
...
for (file in my.files){
  ...
  for (tiss in fgsea.res$pathway){
    df[[tiss]][df$phen == file] <- ifelse(fgsea.res$padj[fgsea.res$pathway == tiss] < 0.05, 1, 0)
  }
}
...
write.table(df, file = "phens_by_tissues_1_0.tsv" ,row.names = FALSE, dec = ".", sep = "\t", quote = FALSE)
