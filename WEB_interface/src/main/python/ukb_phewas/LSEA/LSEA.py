        ...
        with open(path, newline='') as csvfile:
            ...
            for row in my_reader:
                try:
                    row = list([el.replace('(1)', '') for el in row[0].split()])
                    flag = 0
                    if int(SNP_names_list[row[2]].split("_")[0]) == 6 and (
                            int(SNP_names_list[row[2]].split("_")[1]) <= 33775446 and int(
                            SNP_names_list[row[2]].split("_")[1]) >= 28866528 or int(
                            SNP_names_list[row[2]].split("_")[1]) <= 28700446 and int(
                            SNP_names_list[row[2]].split("_")[1]) >= 25000528):
                        flag = 1
                    if flag != 1:
...
def clump_reducing(clumpig_universe):
            ...
            if index1 != index2:
                if clumpig_universe[index1][0] == clumpig_universe[index2][0]:
                    ...
                    if int(getOverlap(a, b)) != 0:
                        if max([int(getOverlap(a, b)) / int(clumpig_universe[index1][2]),
                                int(getOverlap(a, b)) / int(clumpig_universe[index2][2])]) > 0.6:
                            ind_check = 1
        if ind_check == 0:
    ...
    for index1 in list(to_reduce.keys()):
        ...
        if index1 not in passed:
                ...
                if index2 not in passed:
                    if index1 != index2:
                        if to_reduce[index1][0] == to_reduce[index2][0]:
                            ...
                            if int(getOverlap(a, b)) != 0:
                                if max([int(getOverlap(a, b)) / int(to_reduce[index1][2]),
                                        int(getOverlap(a, b)) / int(to_reduce[index2][2])]) >= 0.6:
            ...
            ret_universe[index1] = (
            [to_reduce[index1][0], list(snp_set), int(intervals[1]) - int(intervals[0]), int(intervals[0]),
             int(intervals[1])])
        passed.add(index1)
    return (ret_universe)
...
def create_clumpverse(flag, chr_snp_dict, coord_chr_snp_dict, all_snps, cl_dir='no'):
                        ...
                        if row[2] not in clumpig_universe and "NONE" not in row[11].split(','):
                            ...
                            for snp in row[11].split(','):
                                if snp not in clumpig_universe[row[2]][1]:
                                    clumpig_universe[row[2]][1].append(snp)
                        elif row[2] not in clumpig_universe and "NONE" in row[11].split(',') and row[
                            2] not in NONE_clumps:
    ...
    for index in clumpig_universe.keys():
        ...
        try:
            ...
            pass
    clump_med = median(list([clumpig_universe[index][2] for index in clumpig_universe.keys() if
                             len(clumpig_universe[index][1]) >= 2 and len(clumpig_universe[index]) > 3]))
    for index in NONE_clumps:
        try:
            ...
            for coord in coord_chr_snp_dict[test_clump[0]].keys():
                if int(chr_snp_dict[test_clump[0]][test_clump[1][0]]) - clump_med / 2 <= int(coord) <= int(
                        chr_snp_dict[test_clump[0]][test_clump[1][0]]) + clump_med / 2 and coord != \
                        chr_snp_dict[test_clump[0]][test_clump[1][0]]:
    ...
    for clump in new_verse:
        ...
        for snp in new_verse[clump][1]:
            try:
                if int(SNP_names_list[snp].split("_")[0]) == 6 and (
                        int(SNP_names_list[snp].split("_")[1]) <= 33775446 and int(
                        SNP_names_list[snp].split("_")[1]) >= 28866528 or int(
                        SNP_names_list[snp].split("_")[1]) <= 28700446 and int(
                        SNP_names_list[snp].split("_")[1]) >= 25000528):
                    l2 += 1
            except:
                pass
        if l2 != 0:
...
def make_clumps(input_dict, file_name, plink_dir, bfile):
    ...
    with open(os.getcwd() + "/" + file_name.split('/')[-1].split('.')[0] + '_for_plink.tsv', 'w',
              newline='') as csvfile:
        my_writer = csv.writer(csvfile, delimiter='\t')
        ...
        snp_cmd = subprocess.call(
            '{0}/plink --memory 500 --bfile {1} --clump {2} --clump-field P --clump-p1 1e-04 --clump-r2 0.1 --clump-snp-field SNP  --clump-kb 500 --out {3} --silent 2> {4}'.format(
                plink_dir, bfile, os.getcwd() + "/" + file_name.split('/')[-1].split('.')[0] + '_for_plink.tsv',
                os.getcwd() + "/" + file_name.split('/')[-1].split('.')[0] + '.clumped', 'PLINK_clumping.log'),
            shell=True)


def annotate_vcf(file_name, snpeff, genome):
    print('Annotating file with snpEff')
    snp_cmd = subprocess.call('java -Xmx1g -jar {0} -v {1} {2} > {3} 2> {4}'.format(snpeff + '/snpEff.jar', genome,
                                                                                    os.getcwd() + "/" + file_name,
                                                                                    os.getcwd() + "/" +
                                                                                    file_name.split('.')[
                                                                                        0] + "_annotated.vcf",
                                                                                    'snpeff.log'), shell=True)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-pl', help='PlINK  result of clumping (.clumped file) if it is already prepared',
                        metavar='File',
                        type=str, required=False)
    ...
    parser.add_argument('-bf', help='Bfile for PLINK', metavar='Str',
                        type=str, required=False,
                        default='1000G_20101123_v3_GIANT_chr1_23_minimacnamesifnotRS_CEU_MAF0.01')
    ...
    if not vf and not pl and p:
        ...
        df1 = GSEA(clumpig_universe,
                   get_check(os.getcwd() + "/" + af.split('/')[-1].split('.')[0] + '.clumped.clumped', 'no',
                             SNP_names_list, clumpig_universe), clumpsetbd, genesetdb, coord_gene_dict, 'yes')
        if len(df1) != 0:
            df1.to_csv(af.split('.')[0] + '.enrich.tsv', sep='\t', index=False)
            rename_cmd = subprocess.call(
                "var=\"gene_set\tp-value\tq-value\tenrich_description\";  sed -i \"1s/.*/$var/\" {}".format(
                    af.split('.')[0] + '.enrich.tsv'), shell=True)
        else:
    ...
    if pl:
        df1 = GSEA(clumpig_universe, get_check(pl, 'no', SNP_names_list, clumpig_universe), clumpsetbd, genesetdb,
                   coord_gene_dict, 'yes')
        if len(df1) != 0:
